...
// MY SERVER
Moralis.initialize("XswY5R9kfSQVMk4UzlbtePOEJRparDoFI3YUfpGZ");
...
getNativeBalances = async () => {
    ...
    const polyBalance = await Moralis.Web3API.account.getNativeBalance({ chain: "polygon" });
    ...
    <table class="table">
        ...
        <tbody>
            ...
            <tr>
                <th>POLYGON</th>
                <td>${(polyBalance.balance / 1e18).toFixed(5) + " " + "Poly"} </td>
            </tr>
        </tbody>
    </table>
    `
...
getERC20Balances = async () => {
    ...
    let polyTokens = await Moralis.Web3API.account.getTokenBalances({chain: 'polygon'});
